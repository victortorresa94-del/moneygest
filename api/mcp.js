// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  /api/mcp — MoneyGest MCP server (HTTP streamable)
//  Registra este endpoint en Claude como MCP remoto:
//  URL: https://moneygest-topaz.vercel.app/api/mcp
//  Header: Authorization: Bearer <MONEYGEST_API_KEY>
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        amount:  { type: 'number', description: 'Importe en euros' },
        cycle:   { type: 'string', enum: ['monthly', 'annual', 'weekly', 'quarterly'], description: 'Frecuencia: monthly=mensual, annual=anual, weekly=semanal, quarterly=trimestral' },
  ...
  {
    name: 'list_recurring',
    description: 'Lista los pagos recurrentes (activos e inactivos).',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Añade un ingreso esperado / pendiente de cobrar (factura emitida, deuda a cobrar, etc.).',
    inputSchema: {
      type: 'object',
  ...
  switch (name) {
    ...
    case 'add_transaction': {
      const newTx = {
        ...
        note:        args.note || '',
        profile:     args.profile || 'personal',
      };
      tx.unshift(newTx);
      await dbPatch({ tx });
      return {
        ok: true,
        message: `${args.type === 'income' ? 'Ingreso' : 'Gasto'} de ${fmt(newTx.amount)} añadido: "${newTx.description}" (${fmtDate(newTx.date)})`,
      };
    }
    ...
    case 'list_transactions': {
      ...
      return {
        ...
        transactions: list.map(t => ({
          ...
          importe:     fmt(t.amount),
          fecha:       fmtDate(t.date),
          categoria:   t.category || '—',
          perfil:      t.profile || 'personal',
          nota:        t.note || '',
        })),
      };
    }
    ...
    case 'list_bills': {
      ...
      return {
        ...
        bills: list.map(b => ({
          nombre:    b.name,
          importe:   fmt(b.amount),
          vencimiento: b.dueDate ? fmtDate(b.dueDate) : 'Sin fecha',
          estado:    b.status === 'paid' ? 'Pagado' : 'Pendiente',
          perfil:    b.profile || 'personal',
        })),
      };
    }
    ...
    case 'list_recurring': {
      ...
      return {
        ...
        recurrentes: list.map(s => ({
          nombre:  s.name,
          importe: fmt(s.amount),
          ciclo:   cycleLabel[s.cycle] || s.cycle,
          dia:     `Día ${s.day}`,
          activo:  s.active,
          perfil:  s.profile || 'personal',
        })),
      };
    }
...
export default async function handler(req, res) {
  ...
  // GET — some clients ping to check availability
  if (req.method === 'GET') {
    return res.status(200).json({ name: 'MoneyGest MCP', version: '1.0', status: 'ok' });
  }
